...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        if(Config::get_instance()->get_close_log()==0) {
            LOG_INFO << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        if(Config::get_instance()->get_close_log()==0) {
            LOG_INFO << "setOption sock=" << m_sock
                     << " level=" << level << " option=" << option
                     << " errno=" << errno << " errstr=" << strerror(errno);
        }
        return false;
...
//    m_localAddress = addr;
    ...
    if(addr->getFamily() != m_family) {
        LOG_INFO<<"Socket::bind "<<"addr->getFamily() != m_family     error";
        return false;
    }
...
//            sylar::FSUtil::Unlink(uaddr->getPath(), true);
    ...
    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        LOG_INFO<<"Socket::bind "<<"::bind(m_sock, addr->getAddr(), addr->getAddrLen())  error";
        return false;
    }
    getLocalAddress();
    return true;
